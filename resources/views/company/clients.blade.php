                        ...
                        <tbody>
                            @foreach ($clients as $client)
                                <tr>
                                    ...
                                    <td>{{ $client->is_active ? 'Active' : 'De-activated' }}</td>
                                    <td>
                                        <p>{{ number_format((float) $client->security_benefit, 2) }} EUR</p> <a
                                            href="#" class="btn btn-info" data-toggle="modal"
                                            data-target="#cycleModal">View Cycle</a>
                                                        ...
                                                        <div class="col-md-3 items-cent">
                                                            <div class="modal-body text-center">
                                                                <a type="button" class="btn btn-primary"
                                                                    ...
                                                                    href="{{ url('/company/extend-subscription/' . $client->id) }}">Extend
                                                                    for 365 days</a>
                                                                <a onclick="confirmCancellation('{{ url('/company/disable-client/' . $client->id) }}')"
                                                                    style="margin-top: 5px" class="btn btn-danger">Cancel
                                                                    Subscription</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="payment-div">
                                                        <h4 style="color: black">Monthly Payment:
                                                            {{ number_format($client->monthly_payment, 2) }} EUR</h4>
                                                    </div>
                                    ...
                                    <td id="action" style="margin-top: 46px">
                                        <a href={{ url('/company/single-client/' . $client->id) }}
                                            style="text-decoration: none"><i class="fa-solid fa-eye"
                                                style="color: #ffffff;"></i></a>
                                        <a href={{ url('/company/edit-client/' . $client->id) }}><i
